    ...
    protected function compileButtons($data)
    {
        ...
        if ($prev) {
            if ($prev['type'] == 'default') {
                ...
                $btnPrev = "<button style='float: left;' ".$tabIndexHtml." type='button' data-action='prev' class='".$btnClass."'>" . $prev['text'] . "</button>";
            } else {
        ...
        if ($next) {
            if ($next['type'] == 'default') {
                ...
                $btnNext = "<button style='float: right;' ".$tabIndexHtml." type='button' data-action='next' class='".$btnClass."'>" . $next['text'] . "</button>";
            } else {
                $alt = esc_attr(ArrayHelper::get($next,'img_alt'));
