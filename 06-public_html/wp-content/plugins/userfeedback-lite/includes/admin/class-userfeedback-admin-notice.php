...
final class UserFeedback_Admin_Notice {
	...
	public function display_inline_notice( $name, $title, $message, $type = 'success', $is_dismissible = false, $args = array() ) {
		...
		ob_start();
		?>
		<div class="userfeedback-notice <?php echo 'userfeedback-' . esc_attr( $type ) . '-notice' . esc_attr($dismissible); ?>" data-notice="<?php echo esc_attr( $name ); ?>">
			<div class="userfeedback-notice-icon <?php echo 'userfeedback-' . esc_attr( $type ) . '-notice-icon'; ?>">
			</div>
			<div class="userfeedback-notice-text <?php echo 'userfeedback-' . esc_attr( $type ) . '-notice-text'; ?>">
				<?php
				// Title
				if ( ! empty( $title ) ) {
					?>
					<p class="userfeedback-notice-title"><?php echo esc_html( $title ); ?></p>
					<?php
				}
				...
				if ( ! empty( $message ) ) {
					if ( empty( $args['skip_message_escape'] ) ) {
						?>
						<p class="userfeedback-notice-message"><?php echo esc_html( $message ); ?></p>
						<?php
					} else {
						?>
						<p class="userfeedback-notice-message"><?php echo esc_html($message); ?></p>
